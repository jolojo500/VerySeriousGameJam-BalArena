...
public static class SquashAndStretch
{
    ...
    public static IEnumerator DoSquashAndStretch(MonoBehaviour obj, float scale, float time)
    {
        ...
        Vector3 squashScale = new Vector3(resetScale.x * scale, resetScale.y / scale, resetScale.z * scale);
        // Define the stretch scale (increase y, decrease x and z)
        Vector3 stretchScale = new Vector3(resetScale.x / scale, resetScale.y * scale, resetScale.z / scale);
        ...
        yield return obj.StartCoroutine(ScaleOverTime(obj, resetScale, time / 3));
    }
